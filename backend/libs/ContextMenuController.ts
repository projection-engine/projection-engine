            ...
            (event, data) => {
                ...
                const mapped = template.map(e => {
                    ...
                    if (e.id)
                        return {
                            ...e,
                            click: () => WindowController.window.webContents.send(ROUTES.CONTEXT_MENU_CALLBACK, {
                                id: e.id,
                                group: id
                            })
                        }
    ...
    static #mapMenu(e, parent) {
        return Array.isArray(e.submenu) ? e.submenu.map(s => {
            ...
            if (Array.isArray(s.submenu))
                ...
                newData.click = () => WindowController.window.webContents.send(ROUTES.CONTEXT_MENU_CALLBACK, {
                    id: s.id,
                    group: parent
                })
