...
module.exports = async function parseMaterial(basePath, data, textures, images, partialPath, projectPath, index) {
    ...
    const fallbackValues = SIMPLE_MATERIAL_TEMPLATE.uniformData[2]
    const albedo = SIMPLE_MATERIAL_TEMPLATE.uniformData[3]
    const normal = SIMPLE_MATERIAL_TEMPLATE.uniformData[4]
    const roughness = SIMPLE_MATERIAL_TEMPLATE.uniformData[5]
    const metallic = SIMPLE_MATERIAL_TEMPLATE.uniformData[6]
    const ao = SIMPLE_MATERIAL_TEMPLATE.uniformData[7]
    const emission = SIMPLE_MATERIAL_TEMPLATE.uniformData[8]
    const copy = {...SIMPLE_MATERIAL_TEMPLATE}
