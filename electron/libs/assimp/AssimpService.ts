...
export default class AssimpService extends AbstractSingleton{
	...
	async load(pathToDir, files) {
		...
		for (let j = 0; j < files.length; j++) {
			...
			await this.#load(dir, files[j], buffer)
		}
	}
	...
	async #load(dir, file, bufferData): Promise<void> {
		try {
			...
			await createRegistryEntry(crypto.randomUUID(), collectionPath.replace(ElectronWindowService.getInstance().pathToAssets, ""))
		} catch (err) {
			console.error(err)
		}
	}
	...
	async #createMesh(dir, meshes, index, data, collectionName): Promise<void> {
		...
		try {
			...
			await createRegistryEntry(meshID, localName.replace(ElectronWindowService.getInstance().pathToAssets, ""))
		} catch (err) {
			console.error(err)
		}
	}
