...
export default class IPCEventListener extends AbstractSingleton {
    constructor() {
        ...
        ipcMain.on(IPCRoutes.LOAD_PROJECT_METADATA, this.#loadProjectMetadata)
        ipcMain.on(IPCRoutes.READ_FILE, this.#readFile)
        ipcMain.on(IPCRoutes.SET_PROJECT_CONTEXT, this.#setProjectContext)
        ipcMain.on(IPCRoutes.FILE_DIALOG, this.#fileDialog)
        ipcMain.on(IPCRoutes.OPEN_WINDOW, this.#openWindow)
        ipcMain.on("minimize", this.#minimize)
        ipcMain.on("maximize", this.#maximize)
        ipcMain.on("close", this.#close)
        ipcMain.on(IPCRoutes.OPEN_SELECTION, this.#openSelection)
        ipcMain.on(IPCRoutes.RESOLVE_NAME, this.#resolveName)
        ipcMain.on(IPCRoutes.UPDATE_REG, this.#updateRegistry)
        ipcMain.on(IPCRoutes.CLOSE_EDITOR, this.#openProjectWindow)
        ipcMain.on(IPCRoutes.GET_GLOBAL_SETTINGS, this.#getGlobalSettings)
        ipcMain.on(IPCRoutes.UPDATE_GLOBAL_SETTINGS, this.#updateGlobalSettings)
        ipcMain.on(IPCRoutes.CREATE_REG, this.#createRegistry)
        ipcMain.on(IPCRoutes.REFRESH_CONTENT_BROWSER, this.#refreshContentBrowser)
        ipcMain.on(IPCRoutes.READ_REGISTRY, this.#readRegistry)
        ipcMain.on(IPCRoutes.STORE_UPDATE, this.#storeUpdate)
    }
