...
export default class WindowController {
	...
	static preventAppClosing = false
	...
	static openEditorWindow() {
		...
		WindowController.window.loadFile(path.join(__dirname, "./editor-window.html"))
			.then(() => {
				WindowController.window.webContents.send(IPCRoutes.EDITOR_INITIALIZATION, WindowController.pathToProject)
	...
	static async bindEssentialResources(pathTo: string) {
		...
		WindowController.pathToRegistry = pathTo + path.sep + FileTypes.REGISTRY
		WindowController.metadata = <MutableObject>await readTypedFile(pathTo + path.sep + FileTypes.PROJECT, "json") || {}
		WindowController.registry = <{
            [key: string]: RegistryFile
        }>await readTypedFile(WindowController.pathToRegistry, "json") || {}
		WindowController.pathToProject = pathTo
		WindowController.id = pathTo.split(path.sep).pop()
		WindowController.pathToAssets = path.resolve(pathTo + path.sep + Folders.ASSETS)
		WindowController.pathToPreviews = path.resolve(pathTo + path.sep + Folders.PREVIEWS)
