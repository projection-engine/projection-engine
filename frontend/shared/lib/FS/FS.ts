...
export default class FS {
    static sep:string = <string>Electron.path.sep
    static path?:string
    static TEMP?:string
    static PREVIEW_PATH?:string
    static rootDir = Electron.os.homedir()
    ...
    static initialize(path:string) {
        if (FS.#initialized)
            return
        FS.#initialized = true
        FS.path = Electron.path.resolve(path.replace(PROJECT_STATIC_DATA.PROJECT_FILE_EXTENSION, "") + FS.sep)
        FS.TEMP = Electron.path.resolve(FS.path + FS.sep + PROJECT_FOLDER_STRUCTURE.TEMP + FS.sep)
        FS.PREVIEW_PATH = Electron.path.resolve(FS.path + FS.sep + PROJECT_FOLDER_STRUCTURE.PREVIEWS + FS.sep)
        FS.ASSETS_PATH = Electron.path.resolve(FS.path + FS.sep + PROJECT_FOLDER_STRUCTURE.ASSETS + FS.sep)
    ...
    static resolvePath(path:string):string {
        return Electron.path.resolve(path)
    }
    ...
    static exists(path:string): boolean {
        return Electron.fs.existsSync(Electron.path.resolve(path))
    }
