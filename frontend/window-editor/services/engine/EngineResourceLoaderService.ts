...
export default class EngineResourceLoaderService {
	...
	static async scene(path) {
		const file = await FSFilesService.readFile(FS.ASSETS_PATH + FS.sep + path, "json")
		const entities = []
		const root = EntityAPI.getNewEntityInstance()
		root.name = path.replace(FileTypes.COLLECTION, "").split(FS.sep).pop()
		entities.push(root)
		...
		try {
			if (file) {
				...
				AlertController.error(LocalizationEN.COLLECTION_NOT_FOUND)
		} catch (error) {
			console.error(error)
		}
	}
	...
	static async load(event, asID, mouseX?: number, mouseY?: number) {
		...
		for (let i = 0; i < items.length; i++) {
			...
			case FileTypes.PRIMITIVE: {
				const file = await FSFilesService.readFile(FS.ASSETS_PATH + FS.sep + res.path, "json")
		...
		if (entitiesToPush.length > 0) {
			...
			AlertController.success(LocalizationEN.ENTITIES_CREATED)
		}
	}
