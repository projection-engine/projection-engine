<script>
    ...
    onMount(() => {
        Electron.ipcRenderer.once(ROUTES.GET_GLOBAL_SETTINGS, (_, data) => globalSettings = data)
        Electron.ipcRenderer.send(ROUTES.GET_GLOBAL_SETTINGS)
    })
...
<div data-svelteform="-">
    ...
    <button
            ...
            style="width: 100%"
            on:click={() => Electron.ipcRenderer.send(ROUTES.UPDATE_GLOBAL_SETTINGS, globalSettings)}>
