...
const TEXTURE_TEMPLATE = require("../../../../static/TEXTURE_TEMPLATE")
module.exports = async function loadTexture(basePath, texture, textures, images, partialPath, projectPath, name, index) {
        ...
        if (imgURI !== undefined) {
            ...
            if (typeof imgURI.uri === "string" && imgURI.uri.includes("static:image"))
                file = imgURI.uri
            else {
            ...
            if (file) {
                const imageBase64 =fetched ?  `data:image/${imgURI.uri.split(".").pop()};base64, ` + file : "static:image/png;base64, " + file
