...
import AtmosphereComponent from "@engine-core/components/AtmosphereComponent";
...
export default class LightsAPI extends AbstractSingleton {
    ...
    constructor() {
        ...
        this.#primaryBuffer = <Float32Array>ArrayBufferAPI.allocateVector(UberShader.MAX_LIGHTS * 16, 0, false, false, false)
        this.#secondaryBuffer = <Float32Array>ArrayBufferAPI.allocateVector(UberShader.MAX_LIGHTS * 16, 0, false, false, false)
    }

    static packageLights(keepOld = false, force = false) {
        const instance = this.get<LightsAPI>()
        if (force) {
