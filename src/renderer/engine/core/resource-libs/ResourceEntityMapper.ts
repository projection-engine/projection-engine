...
export default class ResourceEntityMapper extends AbstractSingleton {
    #byComponent = new Map<Components, DynamicMap<string, Entity>>()
    ...
    constructor() {
        ...
        manager.addEventListener("hard-change", (event: EntityListenerEvent<Entity, Components>) => {
            switch (event.type) {
                case "delete": {
                    const withComponent = event.all.filter(e => e.Components.has(component))
                    this.#byComponent.get(withComponent).removeBlock(withComponent, e => e.id)
                    break
                }
                case "create": {
                    const withComponent = event.all.filter(e => e.Components.has(component))
        ...
        Object.values(Components).forEach(c => this.#byComponent.set(c, new DynamicMap<string, Entity>()))
    }

    getListByComponentType(component: Components): Entity[] {
        return this.#byComponent.get(component).array
    }

    getByComponentType(component: Components): DynamicMap<string, Entity> {
    ...
    static addEntity(entity: Entity) {
        if (Engine.environment !== Enviroment.DEV && entity.rigidBodyComponent) {
